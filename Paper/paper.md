...
This software is available in the format of a Python script file. The experimental features are supported only on the Windows operating system due to driver dependencies, while the data processing features are cross-platform and can be run on Windows, macOS, and Linux. It will also pack into executable software for those who do not have experience in Python programs.
